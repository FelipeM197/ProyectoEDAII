...
class GestorGrafico:
    ...
    def cargar_recursos(self):
        ...
        [CORREGIDO]: Ahora todo apunta a 'sprites_finales/img/'.
        """
        # Definimos la ruta única para evitar confusiones
        ruta_img = "sprites_finales/img/"

        try:
            # --- FONDOS Y PERSONAJES BÁSICOS ---
            # Intentamos cargar desde la carpeta final
            ...
            self.assets['jugador1'] = pygame.transform.scale(img_p1, (180, 180))
            ...
            # Caja de texto
            img_caja = pygame.image.load(ruta_img + "cajadeTexto.png").convert_alpha()
            self.assets['caja_texto'] = pygame.transform.scale(img_caja, (900, 120))

            # --- RESTO DE ASSETS (EFECTOS, UI, VARIACIONES) ---
            
            # Variaciones del Boss
            self.assets['boss_dano'] = self.cargar_y_escalar(ruta_img + "boss_dano.png", (280, 280))
            self.assets['boss_atacando'] = self.cargar_y_escalar(ruta_img + "boss_atacando.png", (280, 280))
            ...
            print("ASEGÚRATE DE HABER MOVIDO LOS SPRITES A 'sprites_finales/img/'")
    ...
    def cargar_todos_los_assets(self):
        ...
        ruta_img = "sprites_finales/img/" # Definimos la ruta aquí también
        
        self.assets["fondo_menu"] = self.assets['fondo']
        try:
            nombre_fondo = config.NIVELES[0]['fondo']
            self.assets["fondo_n1"] = pygame.image.load(ruta_img + nombre_fondo).convert()
            self.assets["fondo_n1"] = pygame.transform.scale(self.assets["fondo_n1"], (config.ANCHO, config.ALTO))
        except:
            print(f"No se encontró fondo específico en {ruta_img}, usando default.")
    ...
    def dibujar_interfaz(self, p1, p2, boss, frase_log, sprite_boss_override=None):
        ...
        # --- DIBUJAR PERSONAJES ---
        self.pantalla.blit(self.assets['jugador1'], (150, 480)) 
        self.pantalla.blit(self.assets['jugador2'], (300, 480)) 
        ...
        for i, linea in enumerate(lineas):
            txt = self.fuente_log.render(linea, True, (0, 0, 0))
